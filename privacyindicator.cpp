...
bool PrivacyIndicator::load()
{
    return true;
}

bool PrivacyIndicator::init()
{
...
void PrivacyIndicator::setupTimer()
{
    // 初始获取设备状态
    updateDeviceStatus();
    ...
    connect(m_timer, &QTimer::timeout, this, &PrivacyIndicator::updateDeviceStatus);
    m_timer->start(2000); // 每2秒检测一次
}

void PrivacyIndicator::setVisible(bool visible)
{
    qCDebug(logPrivacy) << "Setting shell plugin visibility - From:" << m_visible << "To:" << visible;
...
QVariantMap PrivacyIndicator::getDeviceStatus()
{
    QVariantMap status;
...
QString PrivacyIndicator::getTooltipText()
{
    QStringList devices;
    if (m_cameraInUse) devices.append(tr("Camera"));
    if (m_microphoneInUse) devices.append(tr("Microphone"));
    if (m_locationInUse) devices.append(tr("Location"));
    
    return devices.isEmpty() ? QString() : tr("Privacy: ") + devices.join(", ");
}

bool PrivacyIndicator::isDeviceInUse(const QString &devicePath)
{
...
bool PrivacyIndicator::checkCameraInUse()
{
    // 检查常见的摄像头设备节点
    QStringList videoDevices;
    QDir devDir("/dev");
    QStringList entries = devDir.entryList(QStringList() << "video*", QDir::System);
    for (const QString &entry : entries) {
...
bool PrivacyIndicator::isAudioInputInUse()
{
    ...
    QStringList entries = asoundDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
    
    for (const QString &entry : entries) {
        if (entry.startsWith("card")) {
            QString subStatusPath = QString("/proc/asound/%1/pcm0c/sub0/status").arg(entry);
...
bool PrivacyIndicator::checkMicrophoneInUse()
{
    return isAudioInputInUse();
}

bool PrivacyIndicator::checkLocationInUse()
{
    // 检查位置服务状态
    // 1. 检查 geoclue 服务是否正在运行且有活动客户端
    QProcess process;
    process.start("systemctl", QStringList() << "--user" << "is-active" << "geoclue.service");
...
void PrivacyIndicator::updateDeviceStatus()
{
    ...
    if (oldMicrophone != m_microphoneInUse) {
        qCDebug(logPrivacy) << "Microphone status changed:" << m_microphoneInUse;
